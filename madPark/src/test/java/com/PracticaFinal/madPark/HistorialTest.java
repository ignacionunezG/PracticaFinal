...
public class HistorialTest {
    ...
    public void usersEndpoinTest() {
        Iterable<Historial> users = repository.findAll();

        String url = "http://localhost:" + Integer.toString(port) + "/api/v1/historial";
        HttpHeaders headers = new HttpHeaders();
        HttpEntity<String> entity = new HttpEntity<>(headers);

        ResponseEntity<Iterable<Historial>> result = testRestTemplate.exchange(
            url,
            HttpMethod.GET,
            entity,
